...
const ModalContent = ({ handleCloseSearch, debouncedSearchKeyword }: modalContentProps) => {
    ...
    return (
        <div className="w-full max-w-screen h-full max-h-screen flex flex-col bg-MAIN_SEARCH bg-center bg-cover bg-no-repeat rounded-3xl xl:rounded-2xl transition-all duration-[1000ms] ease-in-out delay-75">
            ...
            />
            <div
                className={`h-full flex justify-center items-center flex-col transition-all duration-300 ${isFocused ? "pb-[640px]" : "pb-40"}`}
            >
                <ModalTitleSection
                    titleContainerStyle={`w-full flex flex-col items-center py-12 transition-transform duration-[300ms] ease-in-out ${isFocused ? "-translate-y-1" : "translate-y-0"}`}
                />
                <SearchBar
